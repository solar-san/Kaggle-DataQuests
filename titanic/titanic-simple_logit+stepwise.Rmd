...
There are plenty of `NA`s values; however, if we were to remove them now, we would end up with a different number of dimensions compared to the test-set. Because we have (at the moment) viable replacement values, we will use a `merge` to solve this and proceed removing the `NA`.

To improve the performance of the model, we could use a clustering approach to find robust replacement values for `NA`s.
